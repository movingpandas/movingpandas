...
1. [Garcez Duarte, M. M., & Sakr, M. (2025). An experimental study of existing tools for outlier detection and cleaning in trajectories. GeoInformatica, 29(1), 31-51.](https://doi.org/10.1007/s10707-024-00522-y) - *"MovingPandas uses the Kalman Filter (KF) algorithm for outlier detection"*
1. [Alexanne, W., Marchal, F., & Castagnos, S. (2025, June). BIRD: A Museum Open Dataset Combining Behavior Patterns and Identity Types to Better Model Visitors' Experience. In Adjunct Proceedings of the 33rd ACM Conference on User Modeling, Adaptation and Personalization.](https://dl.acm.org/doi/full/10.1145/3708319.3733686) - *"We relied on the MovingPandas library v0.20.0 to normalize the trajectories with an interval of two seconds between two different positions." "From each trajectory, we can extract a set of features to further analyze the visitor’s behavior by relying on MovingPandas."*
1. [Molina-Toledo, L., Loaiza, S. E., García, J. A., Carrión, G., Gómez-Ayora, A., & Orellana, D. (2025). Negotiating space: The daily mobility of women waste pickers in Cuenca, Ecuador. Journal of Transport Geography, 128, 104378.](https://www.sciencedirect.com/science/article/pii/S0966692325002698) *"MovingPandas was applied for trajectory smoothing and compression. [...] MovingPandas was then used again to aggregate the trajectories and identify flows and clusters."*
1. [Wang, L., & Kwan, M. P. (2026). Profiling Spatiotemporal Contexts Associated with Compound Exposure Disadvantages in Noise and Greenery: A Study Using Mobile Sensors and Explainable Machine Learning. Social Science & Medicine, 119254.](https://www.sciencedirect.com/science/article/pii/S0277953626003308) *"GPS trajectory processing was performed using the Python package MovingPandas."*
1. [Yang, Z., & Kwan, M. P. (2026). The interplay between actual and perceived air pollution, physical activity, and stress across mobility-based geographic contexts: A double machine learning approach. Social Science & Medicine, 119499.](https://www.sciencedirect.com/science/article/pii/S0277953626005757) *"First, we processed real-time GPS records using the MovingPandas Python library to identify “activities” (defined as stationary points lasting >15 minutes within a 100-meter radius) and “travels” (movement between activities)"*
1. [Xin, Q., Li, Z., Zhang, M., & Hu, Y. (2026). Trajectory data-driven analysis of behavioral patterns of ship lockage. Ocean Engineering, 364, 126819.](https://www.sciencedirect.com/science/article/pii/S0029801826026533) *"Trajectory data processing in this study is conducted using the MovingPandas library. First, discrete ship trajectory points are converted into trajectory linestrings. Shorter trajectories are then discarded based on desired minimum duration of trajectories. Additionally, trajectories are segmented into subtrajectories whenever there is a gap in the observations. [...] To support subsequent analysis, each trajectory is enriched with computed attributes such as speed, acceleration, direction, time delta, and distance delta, derived from sequential trajectory points."*
1. [Nam, J., & Jung, P. H. (2026). Tracing carbon footprints from vessel footprints: Movement analytics for localized vessel carbon emissions estimation using AIS trajectory data. Ocean & Coastal Management, 281, 108324.](https://www.sciencedirect.com/science/article/pii/S0964569126002334). *"the collected AIS raw data are processed using the Python library MovingPandas [...] To eliminate positional outliers and excessive speed values, we applied the IqrCleaner class"* <br/> <img  width="400"  alt="Nam2026_Fig2" src="https://github.com/user-attachments/assets/b815270b-b30f-4196-861a-5bf6379d79c3" />
...
[![WorkshopVideo](https://user-images.githubusercontent.com/590385/67161044-f08cb100-f356-11e9-8799-f972175ec7f4.png)](http://www.youtube.com/watch?v=qeLQfnpJV1g "Watch the OpenGeoHub Summer School 2019 recording")
