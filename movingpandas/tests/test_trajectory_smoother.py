...
class TestTrajectorySmoother:
    ...
    def test_kalman_smoother_cv_single(self):
        ...
        truth = loads('LINESTRING (1.7982884510469304 0.6585353696461531, 1.7986579478436395 0.6588189259897214, 1.7996332413712328 0.6596163790136829, 1.8009712874015955 0.6608076466642573, 1.8024490723366013 0.6622216786861853)')
        assert np.allclose(result.to_linestring().xy, truth.xy)

    @requires_stonesoup
    def test_kalman_smoother_cv_single_geo(self):
        from movingpandas.trajectory_smoother import KalmanSmootherCV
        traj = make_traj([Node(), Node(1, 0.1, second=1), Node(2, 0.2, second=2), Node(3, 0, second=3), Node(3, 3, second=4)], crs=CRS_LATLON)
        result = KalmanSmootherCV(traj).smooth(process_noise_std=0.1, measurement_noise_std=10)
        truth = loads('LINESTRING (1.7982884510469301 0.6587980686797733, 1.7986579478436397 0.6590817401695466, 1.7996332413712322 0.6598795189933684, 1.8009712874015955 0.6610712754609851, 1.8024490723366011 0.6624858873670953)')
        assert np.allclose(result.to_linestring().xy, truth.xy)
    ...
    def test_kalman_smoother_sv_collection(self):
        ...
        truth_1 = loads('LINESTRING (0.0044805333934456 0.0000761144223231, 5.9954842501546315 -0.0000845436868686, 6.00040594977978 6.000156500844182, 8.999629266671697 8.999851928420188)')
        truth_2 = loads('LINESTRING (10.004429320057262 10.000008962523324, 15.995566251010434 9.999990965241574, 16.000875056273347 16.000014621277007, 189.9991293726476 18.99998545095791)')
