...
class TrajectoryCollectionSampler:
    def __init__(self, traj_collection, tolerance=timedelta(seconds=1)):
        self.traj_collection = traj_collection
        self.tolerance = tolerance

    def get_sample(self, past_timedelta, future_timedelta, min_meters_per_sec=0.3, randomize=False, future_traj_duration=timedelta(hours=0), fixed_seed=False):
        samples = []

        for traj in self.traj_collection.trajectories:
            sampler = TrajectorySampler(traj, self.tolerance)
            try:
                sample = sampler.get_sample(past_timedelta, future_timedelta, min_meters_per_sec, randomize, future_traj_duration, fixed_seed)
            except RuntimeError:
                continue
            if sample:
...
class TrajectorySampler:
    ...
    def _match_sample_pattern_to_df(self, df, first_move_time, past_timedelta, future_timedelta, randomize, fixed_seed=False):
        ...
        if randomize:
            ...
            if fixed_seed:
                random.seed(number_of_retries+fixed_seed)
    ...
    def get_sample(self, past_timedelta, future_timedelta, min_meters_per_sec=0.3, randomize=False, future_traj_duration=timedelta(hours=0), fixed_seed=False):
        ...
        traj.df.assign(prev_pt=traj.df.geometry.shift())
        if 't' in traj.df.columns:
            times = traj.df.t
        else:
            times = traj.df.reset_index().t
        traj.df = traj.df.assign(delta_t=times.diff().values)
        ...
        successful, start_time, past_time, future_time, start_timedelta = self._match_sample_pattern_to_df(
            df, first_move_time, past_timedelta, future_timedelta, randomize, fixed_seed)
